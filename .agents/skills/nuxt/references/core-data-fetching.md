...
Nuxt provides composables for SSR-friendly data fetching that prevent double-fetching and handle
hydration.
...
**Important**: Don't use `$fetch` alone in setup for initial data - it will fetch twice (server +
client). Use `useFetch` or `useAsyncData` instead.
